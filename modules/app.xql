...
(: one day the damn variable shall be replacd by a function of the same name :)
(:declare function app:files($nodes as node()*) as item()* {
for $fig in data($app:pages//*[@role="main"]/@id)
    order by $fig ascending
    return $fig
    };    :)
...
(:declare
    %templates:wrap
function app:loadScritps($node as node(), $model as map(*)) {

let $scripts := doc('/db/apps/d3ia/data/scritps.xml')//scripts/*
...
return
    <script xmlns="http://www.w3.org/1999/xhtml" 
        ...
        src="{data($scripts[@id = $node/ancestor::xhtml:html//xhtml:div[@role="main"]/@id]/@src)}"/>
};:)
