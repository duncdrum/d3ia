...
function hover(hoverD) {
    ...
    d3.selectAll("rect.bar")
    .filter(function (d) { return d.values.indexOf(hoverD) > -1 })
    .style("fill", "#94B8FF");
}
...
function createBrush(incData) {
    timeRange = d3.extent(incData
    .map(function (d) { return new Date(d.timestamp) }));
    ...
    function brushed() {
        ...
        .style("display", 
            function (d) { return new Date(d.timestamp) >= e[0] && new Date(d.timestamp) <= e[1] ? "block": "none" });
...
/*    console.log(Math.floor((rightSize[0] + leftSize[0]) / 100))*/
    ...
    timeScale
    .range([0, rightSize[0] + leftSize[0] - 10]);
