...
function legendComponent(data) {
    ...
    d3.select("svg").append("g").attr("transform", "translate(50,50)").selectAll("circle").data(griddedData).enter().append("circle").attr("cx", function (d) {
        return d.x
    }).attr("cy", function (d) {
        return d.y
    }).attr("r", 10).style("fill", "lightgray").style("stroke", "black").style("stroke-width", "1px") 
    var sizeExtent = d3.extent(griddedData, function (d) {
        return d.size
    }) 
    countryColor = d3.scale.quantile().domain(sizeExtent).range(colorbrewer.Reds[7]);
    d3.selectAll("circle").style("fill", function (d) {
        return countryColor(d.size)
    });
    newLegend = d3.svg.legend().scale(countryColor).title("Country Size").formatter(d3.format(".2f")).unitLabel("Steradians");
    d3.select("svg").append("g").attr("transform", "translate(50,400)").attr("id", "legend").call(newLegend) 
...
d3.svg.legend = function () {
    ...
    function legend(gSelection) {
        ...
        gSelection.selectAll("rect").data(data).enter().append("rect").attr("height", size[1]).attr("width", function (d) {
        ...
        gSelection.selectAll("line").data(data).enter().append("line").attr("x1", function (d) {
            return xScale(d.domain[0])
        }).attr("x2", function (d) {
            return xScale(d.domain[0])
        }).attr("y1", 0).attr("y2", size[1] + 5).style("stroke", "black").style("stroke-width", "2px") 
        gSelection.selectAll("text").data(data).enter().append("g").attr("transform", function (d) {
            return "translate(" + (xScale(d.domain[0])) + "," + (size[1] + 20) + ")"
        }).style("text-anchor", "middle").append("text").text(function (d) {
            return numberFormat(d.domain[0])
        }) 
        gSelection.append("text").attr("transform", function (d) {
    ...
    function createLegendData(incScale) {
        ...
        for (x in rangeArray) {
            ...
            data.push({
                color: colorValue, domain: domainValues
            })
        }
    }
...
d3.svg.legend0 = function () {
    function legend(gSelection) {
        ...
        gSelection.selectAll("rect").data(testData).enter().append("rect").attr("height", 20).attr("width", 20).attr("x", function (d, i) {
            return i * 25
        }).style("fill", "red") 
        return legend;
    }
...
function legendOver(d) {
    console.log(d) 
    d3.selectAll("circle").style("opacity", function (p) {
