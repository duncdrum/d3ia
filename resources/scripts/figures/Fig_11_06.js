sampleData = d3.range(2000).map(function (d) {
    ...
    datapoint.id = "Sample Feature " + d; datapoint.type = "Feature"; datapoint.properties = {
    };
    datapoint.geometry = {
    };
    datapoint.geometry.type = "Polygon"; datapoint.geometry.coordinates = randomCoords();
    return datapoint;
}) 
...
function createMap(countries) {
    ...
    mapZoom = d3.behavior.zoom().translate(projection.translate()).scale(projection.scale()).on("zoom", zoomed).on("zoomstart", zoomInitialized).on("zoomend", zoomFinished);
