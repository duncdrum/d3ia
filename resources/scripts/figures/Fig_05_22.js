...
function dataViz(incData) {
    expData = incData;
    stackData =[];
    var xScale = d3.scale.linear().domain([1, 10]).range([20, 440]);
    var yScale = d3.scale.linear().domain([0, 70]).range([480, 0]);
    var heightScale = d3.scale.linear().domain([0, 70]).range([0, 480]);
    ...
    stackLayout = d3.layout.stack().values(function (d) {
        return d.values;
    });
